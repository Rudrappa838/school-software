...
const StaffAttendanceMarking = () => {
    ...
    const fetchAttendance = async () => {
        try {
            const res = await api.get('/staff/attendance/daily', { params: { date } });
            setStaff(res.data);
            const map = {};
    ...
    const handleSave = async () => {
        try {
            const payload = Object.entries(attendance).map(([id, status]) => ({ staff_id: parseInt(id), status }));
            await api.post('/staff/attendance', { date, attendanceData: payload });
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-[11px] tracking-wider border-b border-slate-100">
                            <tr><th className="p-4 pl-6">Staff Details</th><th className="p-4">Attendance Status</th></tr>
                        </thead>
                            ...
                            {staff.map(t => (
                                <tr key={t.id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="p-4 pl-6">
                                        ...
                                        <div className="text-xs text-slate-400 font-medium">{t.role} • {t.phone}</div>
                                    </td>
                                    <td className="p-4">

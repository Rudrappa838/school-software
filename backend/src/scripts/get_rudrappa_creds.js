...
const fs = require('fs');
const outputFile = 'rudrappa_creds.txt';

async function getStudentCreds() {
    try {
        // Search by name (case insensitive partial match)
        const nameQuery = 'rudrappa';

        const studentRes = await pool.query(`
            SELECT s.id, s.name, s.admission_no, s.email, s.school_id
            FROM students s
        ...
        if (studentRes.rows.length > 0) {
            ...
            if (userRes.rows.length === 0) {
                ...
                const patternEmail = `${student.admission_no.toLowerCase()}@student.school.com`;
                ...
                if (userRes.rows.length > 0) {
                    output += `(Found via generated email pattern: ${patternEmail})\n`;
                }
            }

            if (userRes.rows.length > 0) {
                output += `=== User Credentials ===\n`;
                output += `Login Email/Username: ${userRes.rows[0].email}\n`;
            ...
            output += `No student found with name containing '${nameQuery}'.\n`;
        }
